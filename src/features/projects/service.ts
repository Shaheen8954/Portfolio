...
import skillsJson from '@/collections/skills.json';

// Resolve asset paths from JSON to ImageMetadata using Vite glob imports
const imageModules = import.meta.glob('/src/assets/**/*.{png,jpg,jpeg,webp,svg}', {
  eager: true,
});

function resolveImage(path?: string): any {
  if (!path) return undefined;
  const mod = imageModules[path] as any;
  return mod?.default;
}

const projectsListUnsorted: Array<ProjectData> = (projectsJson.projects || []).map(
  (p: any) => {
    return {
      id: p.id,
      slug: p.slug,
      imageUrl: resolveImage(p.imageUrlPath),
      projectUrl: p.projectUrl || undefined,
      codeUrl: p.codeUrl || undefined,
      tags: p.tags || [],
      category: p.category || undefined,
      date: p.date,
      galleryImages: (p.galleryImages || []).map((gi: any) => ({
        id: gi.id,
        src: resolveImage(gi.srcPath)!,
      })),
